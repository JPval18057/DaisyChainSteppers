...
void recepcionComandos(){
  while (Serial.available() > 0 && datosNuevos == false){
    ...
    } //      si inData es el comando o si inData es una letra entre a y z se empieza a recibir los datos
    else if ((inData == comandoConfig || inData == comandoDiagnostico || (inData >= 'a' && inData <= 'z'))){
      //Serial.write(primerMarcador);
      recibiendo = true;
      primerMarcador = inData; //se guarda como primer marcador la letra del comando que acaba de entrar
    }
  }
  Serial.flush(); //waits until the data finishes sending, I think this is the part where the arduino just sends whatever it received
  datosNuevos = false; //this indicates that no data is entering
}
